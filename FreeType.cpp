...
bool TVPAddFontToFreeType( const ttstr& storage, std::vector<tjs_string>* faces ) {
	...
	return FreeTypeFaceList->LoadFont( tjs_string(storage.c_str()), faces );
}
//---------------------------------------------------------------------------
...
bool tTVPFreeTypeFaceList::LoadFont( tjs_string filename, std::vector<tjs_string>* faces ) {
	...
	// フェイス名一覧を取得する
	tGenericFreeTypeFace ftf( filename.c_str(), faces_, faces );
	return true;
}
//---------------------------------------------------------------------------
...
tBaseFreeTypeFace* tTVPFreeTypeFaceList::GetFace( const tjs_string& facename, tjs_uint32 options ) const {
	if( !faces_.empty() ) {
		...
		if( f != faces_.end() ) {
			FontInfo* font = *f;
			return new tGenericFreeTypeFace( ttstr( font->filename.c_str() ), TVP_FACE_OPTIONS_FACE_INDEX( font->index ), font->file.get() );
		}
		// スタイル気にせず検索する
		...
		if( i != systemfont_.end() ) {
			...
			return new tGenericFreeTypeFace( font->path, ttstr( facename.c_str() ), font->index );
		}
	}
	return nullptr;
}
...
void tTVPFreeTypeFaceList::GetFontList(std::vector<ttstr> & list, tjs_uint32 flags, const tTVPFont & font ) {
	for( auto i = faces_.begin(); i != faces_.end(); i++ ) {
		if( CheckFaceFlag( *(*i), flags ) ) {
			ttstr facename( (*i)->facename.c_str() );
...
tGenericFreeTypeFace::tGenericFreeTypeFace(const ttstr &fontname, tjs_uint32 options, tTJSBinaryStream* file = NULL) : File(NULL)
{
	...
	try {
		...
		if (!file)
		{
			...
			if( File == NULL ) {
				TVPThrowExceptionMessage( TJS_W("Can't open font file '%1$s'"), FontPath );
			}
		}
		else
		{
...
tGenericFreeTypeFace::tGenericFreeTypeFace(const std::string& path, const ttstr &fontname, tjs_uint32 index ) : File(nullptr), Face(nullptr)
{
	...
	try {
		...
		FaceNames.push_back( fontname.c_str() );
	}
	catch(...)
	{
		throw;
...
tGenericFreeTypeFace::tGenericFreeTypeFace(const ttstr &filename, std::vector<FontInfo*>& fonts, std::vector<tjs_string>* faces ) : File(nullptr), Face(nullptr)
{
	...
	try {
		...
		args.stream = &Stream;
		tjs_string fontname = filename.c_str();
...
tFreeTypeFace::tFreeTypeFace(const std::vector<tjs_string> &fontname, tjs_uint32 options)
{
	...
	for( tjs_int i = 0; i < (tjs_int)count; i++ ) {
		...
		if( Faces[i]->Face == nullptr )
		{
			...
			{
				// ファイルを開く
				Faces[i]->Face.reset( new tGenericFreeTypeFace( fontname[i].c_str(), options ) );
				// 例外がここで発生する可能性があるので注意
			}
			else
			{
				// ネイティブのフォント名による指定 (プラットフォーム依存)
				Faces[i]->Face.reset( new tNativeFreeTypeFace( fontname[i].c_str(), options ) );
				// 例外がここで発生する可能性があるので注意
			}
		}
#else
		...
		if( Faces[i]->Face == nullptr )
		{
			TVPThrowExceptionMessage(TJS_W("Font '%1$s' cannot be used"), fontname[i].c_str() );
		}
